...
class TodoList:
    ...
    def mark_as_complete(self, description: str) -> None:
...
# %% [markdown]
# ### 2. Die Unit-Tests, die die Implementierung vorantreiben

# %%
def test_new_list_is_empty():
    todo_list = TodoList()
    assert len(todo_list.get_tasks()) == 0, "New list should be empty"
    print("Unit test 'test_new_list_is_empty' passed.")
...
# %%
def test_add_task_adds_correct_task():
    todo_list = TodoList()
    todo_list.add_task("Test adding a task")
    assert len(todo_list.get_tasks()) == 1, "List should have 1 task after adding"
    assert todo_list.get_tasks()[0].description == "Test adding a task"
    print("Unit test 'test_add_task_adds_correct_task' passed.")
...
# %%
def test_task_is_not_completed_by_default():
    task = Task("Test task")
    assert task.completed is False, "New task should not be completed by default"
    print("Unit test 'test_task_is_not_completed_by_default' passed.")
...
def test_mark_task_as_complete():
    todo_list = TodoList()
    todo_list.add_task("Task to be completed")
    todo_list.mark_as_complete("Task to be completed")
    assert todo_list.get_tasks()[0].completed is True, "Task should be marked complete"
    print("Unit test 'test_mark_task_as_complete' passed.")
...
# %% [markdown]
# ### 3. Der Akzeptanztest, der das gesamte Szenario überprüft
#
# - Das ist unser Akzeptanztest von oben.
# - Er wird jetzt laufen, weil wir die Teile implementiert haben.


# %%
def test_full_todo_list_scenario_solution():
    # Arrange
    todo_list = TodoList()
