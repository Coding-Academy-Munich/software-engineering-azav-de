...
# ## Hinweise
#
# ### Zu Schritt 1: Serialisierung
#
# Für `_recipe_to_dict`: Erstellen Sie ein Dictionary mit allen Feldern des
# Rezepts. Zutaten werden mit einer List Comprehension konvertiert:
#
# ```python
...
# ```
#
# Für `_dict_to_recipe`: Das Gegenteil — Strings zurück in Enum-Werte, Listen
# in Tupel:
#
# ```python
# ingredients=tuple(
...
#         name=i["name"],
#         amount=i["amount"],
#         unit=Unit(i["unit"]),  # "g" → Unit.GRAM
#     )
#     for i in data["ingredients"]
# )
# ```
#
# `Unit(i["unit"])` funktioniert, weil das Enum String-Werte hat: `Unit("g")`
# ergibt `Unit.GRAM`.
#
# ### Zu Schritt 2: `JsonStore`
#
# Die grundlegende Strategie: bei jeder Operation die gesamte Datei laden,
# ändern, und zurückschreiben. Das ist bei kleinen Datenmengen völlig in Ordnung.
#
# Interne Hilfsmethoden zum Laden und Speichern der rohen Liste:
#
# ```python
...
# ```
#
# `ensure_ascii=False` sorgt dafür, dass Umlaute nicht escaped werden.
#
# Für `save_recipe`: Laden, prüfen ob Titel schon existiert, anhängen,
# speichern.
#
# Für `delete_recipe`: Laden, mit List Comprehension filtern (alle behalten, die
# **nicht** den gesuchten Titel haben), prüfen ob sich die Länge geändert hat,
# speichern.
#
# ### Zu Schritt 3: Tests mit `tmp_path`
#
# Die `tmp_path`-Fixture stellt einen `Path` zu einem temporären Verzeichnis
# bereit. Jeder Test bekommt sein eigenes Verzeichnis:
#
# ```python
# def test_example(tmp_path):
#     json_path = tmp_path / "recipes.json"
#     store = JsonStore(json_path)
#     # json_path existiert anfangs nicht → leere Liste
#     assert store.get_all_recipes() == []
# ```
#
# Testen Sie den Round-Trip besonders gründlich: Alle Felder eines Rezepts
# (inklusive Zutatenmengen, Einheiten und Tags) müssen nach dem Speichern und
# Laden identisch sein.
