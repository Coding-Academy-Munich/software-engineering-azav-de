...
# ## Hinweise
#
# ### Zu Schritt 1: `models/` erstellen
#
# Erstellen Sie zuerst das Verzeichnis und die `__init__.py`:
#
# ```bash
# mkdir src/pykochbuch/models
# touch src/pykochbuch/models/__init__.py
# ```
#
# Verschieben Sie dann die Dateien (z.B. mit `mv` oder im Dateimanager). Passen
# Sie die Imports in den verschobenen Dateien an:
#
# ```python
# # In models/ingredient.py:
# # Vorher: from pykochbuch.unit import Unit
# # Nachher:
# from pykochbuch.models.unit import Unit
# ```
#
# `recipe.py` importiert `Ingredient`, also muss auch dieser Import aktualisiert
# werden.
#
# ### Zu Schritt 2: `storage/` aufteilen
#
# Beginnen Sie mit `base.py` — der abstrakten Klasse. Diese hat keine
# Abhängigkeiten zu anderen Storage-Modulen:
#
# ```python
# # storage/base.py
...
# ```
#
# Dann `serialization.py` mit den Hilfsfunktionen. Dann die einzelnen Stores,
# die jeweils `base.py` und `serialization.py` importieren.
#
# ### Zu Schritt 3: Imports schrittweise aktualisieren
#
# Arbeiten Sie Datei für Datei. Der sicherste Ablauf:
#
# 1. Verschieben Sie eine Datei
# 2. Aktualisieren Sie deren interne Imports
# 3. Aktualisieren Sie alle Dateien, die diese Datei importieren
# 4. `uv run pytest` ausführen
# 5. Erst wenn alles grün ist, die nächste Datei verschieben
#
# Typische Import-Fehler zeigen sich als `ModuleNotFoundError` oder
# `ImportError` — die Fehlermeldung sagt Ihnen genau, welcher Import nicht mehr
# stimmt.
#
# ### Zu Schritt 4: Re-Exports
#
# Die `__init__.py` eines Pakets wird ausgeführt, wenn Sie `from paket import`
# schreiben. Wenn sie Re-Exports enthält, können Sie kürzer importieren:
#
# ```python
# # In src/pykochbuch/models/__init__.py:
# from pykochbuch.models.ingredient import Ingredient
# from pykochbuch.models.recipe import Recipe
# from pykochbuch.models.unit import Unit
# ```
...
# Damit funktioniert:
# ```python
# from pykochbuch.models import Ingredient, Recipe, Unit
# ```
#
# Statt:
# ```python
