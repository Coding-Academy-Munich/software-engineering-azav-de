...
#
# Für `save_account_to_json`: Bauen Sie ein Dictionary mit dem Kontonamen und
# einer Liste von Transaktions-Dictionaries. Verwenden Sie eine List
# Comprehension:
#
# ```python
# data = {
...
# ```
#
# Schreiben Sie mit `path.write_text(json.dumps(data, indent=2))` oder mit
# `open()` und `json.dump()`.
#
# Für `load_account_from_json`: Lesen Sie die Datei, erstellen Sie ein
# `Account`-Objekt und fügen Sie Transaktionen hinzu:
#
# ```python
...
# ```
#
# `TransactionType("Income")` funktioniert, weil die Enum-Werte Strings sind.
